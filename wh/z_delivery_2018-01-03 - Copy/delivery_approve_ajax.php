...
switch($s_userGroupCode){
	case 'it' : case 'admin' : case 'warehouseAdmin' : 
		break;
	default : 
		//return JSON
...
try{
	...
	$sql = "UPDATE stk_bal sb
			INNER JOIN delivery_detail dd on dd.prodCode=sb.prodCode 
	...
	$sql = "INSERT INTO stk_bal (prodCode, sloc, delivery, sales, balance) 
			SELECT dd.prodCode,'8', dd.qty, -1*dd.qty, -1*dd.qty  FROM delivery_detail dd 
			WHERE dd.doNo=:nextNo 
			AND dd.prodCode NOT IN (SELECT sb.prodCode FROM stk_bal sb)
			";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':nextNo', $nextNo);
    $stmt->execute();
