...
try{
	...
	$sql = "UPDATE stk_bal sb
			INNER JOIN sale_detail sd on sd.prodCode=sb.prodCode AND sb.sloc=8
	...
	$sql = "INSERT INTO stk_bal (prodCode, sloc, sales) 
			SELECT sd.prodCode,8, sd.qty FROM sale_detail sd 
			WHERE sd.soNo=:soNo 
			AND sd.prodCode NOT IN (SELECT sb.prodCode FROM stk_bal sb WHERE sb.sloc=8 )
			";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':soNo', $soNoNext);
    $stmt->execute();
